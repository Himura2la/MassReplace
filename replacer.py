...
with open(config['rules_file'], "r") as f: lines = f.readlines()
                ...
                try:
                    ...
                    if buffer != source:
                        if config['test_mode']:
                            ...
                            tmpTgtPath = os.path.join(config['test_target_dir'], relative_path)
                            with open(tmpTgtPath, "w") as f: f.write(buffer)
                        else:
                            pass
                            with open(path, "w") as f: f.write(buffer)  # !!! BEWARE ERRORS !!!!
                        changed_files += 1
                    current_file += 1
